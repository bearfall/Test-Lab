...
public class TestCharactersManager : MonoBehaviour
{
    ...
    public List<TestEnemyCharacter> testEnemyCharacters;
    ...
    void Start()
    {
        ...
        testEnemyCharacters = new List<TestEnemyCharacter>();
        charactersParent.GetComponentsInChildren(testEnemyCharacters);
    ...
    public TestEnemyCharacter GetEnemyCharacterDataByPos(float xPos, float zPos)
    {
        // 検索処理
        // （使用foreach對地圖中的所有角色數據一一做同樣的處理）
        foreach (TestEnemyCharacter charaData in testEnemyCharacters)
        {
            // 檢查角色的位置是否與指定位置匹配
            if ((charaData.xPos == xPos) && // 相同的 X 位置
                (charaData.zPos == zPos)) // 相同的 Z 位置
	...
	/// </summary>
	/// <param name="charaData">対象キャラデータ</param>
	public void DeleteEnemyCharaData(TestEnemyCharacter charaData)
    {
        // リストからデータを削除
        testEnemyCharacters.Remove(charaData);
        // オブジェクト削除
        DOVirtual.DelayedCall(
            0.5f, // 遅延時間(秒)
            () =>
